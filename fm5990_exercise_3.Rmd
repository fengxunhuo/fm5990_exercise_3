...
# All the underlyings which have same tarde date have the same expiration. But is this true in the real market?
...
(df_market_history %>% 
  group_by(trade_date) %>% 
  summarize(num_exps = n_distinct(expiration)) %>% 
  arrange(desc(num_exps)))[1,]
# I use a stupid way to find it, and I do not know it is right haha.
...
df_market_history %>% 
  group_by(trade_date) %>% 
  summarise(exp_date = min(expiration)) %>% 
  arrange(trade_date)

View(df_market_history %>% 
  group_by(trade_date) %>% 
  summarise(exp_date = max(expiration)) %>% 
  arrange(trade_date))
...
df_market_history %>% filter(type == "put") %>% 
                      filter(strike < implied_forward)
...
bizdays(as.character(SPY_put_166$trade_date[1]), 
        as.character(SPY_put_166$trade_date[length(SPY_put_166$trade_date)]), 
        "Brazil/ANBIMA") == length(SPY_put_166$trade_date)
